...
const AddBillModal: React.FC<AddBillModalProps> = ({ visible, onClose, onSave, initialData }) => {
    ...
    // Fetch all existing party names from Firebase
    useEffect(() => {
        const fetchPartyNames = async () => {
            try {
                ...
                querySnapshot.forEach((doc) => {
                    const data = doc.data();
                    if (data.partyName) {
                        parties.add(data.partyName);
                    }
                });
                setAllPartyNames(Array.from(parties).sort());
            } catch (error) {
    ...
    const renderSuggestion = ({ item }: { item: string }) => (
        ...
        >
            <Icon name="history" size={16} color="#FFA4A4" style={styles.suggestionIcon} />
